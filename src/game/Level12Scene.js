...
export class Level12Scene {
  ...
  init() {
    ...
    // Even brighter for morning/sunrise
    this.renderer.toneMappingExposure = 1.3;

    this.scene = new THREE.Scene();
    // Beautiful morning sky colors
    this.scene.background = new THREE.Color(0xFFE4B5); // Moccasin/sunrise tone
    this.scene.fog = new THREE.FogExp2(0xFFE4B5, 0.005);

    this.camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
    
    this.firstPersonFlashlight = new FirstPersonFlashlight(this.scene, this.camera);
    this.firstPersonFlashlight.toggle(false); // Day mode
    ...
    setTimeout(() => {
      if (!gameState.state.level12Started) {
  ...
  setupLighting() {
    ...
    // Warm sunrise hemisphere
    this.hemiLight = new THREE.HemisphereLight(0xffddaa, 0x556655, 0.8);
    this.hemiLight.position.set(0, 50, 0);
    this.scene.add(this.hemiLight);

    // Golden morning sun
    this.sunLight = new THREE.DirectionalLight(0xffcc88, 1.5);
    this.sunLight.position.set(40, 30, 40);
  ...
  buildEnvironment() {
    ...
    // Broken open walls to let morning light in
    const wallGeo = new THREE.CylinderGeometry(arenaRadius + 1, arenaRadius + 1, 4, 32, 1, true);
  ...
  buildFriend() {
    // Friend standing and ready
    const friendGeo = new THREE.CylinderGeometry(0.4, 0.4, 1.8, 8);
    const friendMat = new THREE.MeshStandardMaterial({ color: 0x2244aa });
    this.friendMesh = new THREE.Mesh(friendGeo, friendMat);
    
    // Standing near the exit path
    this.friendMesh.position.set(-2, 0.9, -20);
    this.friendMesh.castShadow = true;
    this.scene.add(this.friendMesh);

    this.colliders.push({ type: 'box', minX: -3, maxX: -1, minZ: -21, maxZ: -19 });

    this.interactables.push({
      mesh: this.friendMesh,
      prompt: '[E] SPEAK WITH FRIEND',
      range: 4,
      action: () => {
        gameState.triggerGuideDialogue({
          speakerName: 'FRIEND',
          message: 'It is over. The sun is rising outside. Let\'s go home.'
        });
      }
    });
  }
  ...
  buildGuardian() {
    ...
    // Guardian remaining at the altar
    this.guardianGroup.position.set(0, 3, 0); 
    ...
    this.guardianLight = new THREE.PointLight(0x88ccff, 2, 15);
    ...
    this.interactables.push({
      ...
      action: () => {
        gameState.triggerGuideDialogue({
          ...
          message: 'My purpose here is fulfilled. I will remain to protect this sanctuary. Go in peace.'
        });
      }
    });
  }
  ...
  buildExit() {
    const exitGeo = new THREE.BoxGeometry(6, 8, 1);
    ...
    this.exitPortal.position.set(0, 4, -40);
  ...
  updateRaycaster() {
    ...
    if (this.hoveredItem) {
      ...
      if (gameState.state.toastMessage !== this.hoveredItem.prompt) {
          // Toast or HUD prompt can be linked if desired
      }
    } else {
  ...
  checkExit() {
    if (this.exitPortal && !gameState.state.finalExitReached) {
      const dist = this.player.position.distanceTo(this.exitPortal.position);
      if (dist < 4.0) {
        gameState.triggerFinalExitL12();
      }
    }
  }
